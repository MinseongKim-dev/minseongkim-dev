...
export function RelationshipsView() {
  ...
  const handleAddMeeting = async (e: React.FormEvent) => {
    ...
    setMeetingNotes('');
    if (isMobile) setShowMeetingForm(false);
  };
  ...
  const ContactForm = (
    <form onSubmit={handleAddContact} style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
      ...
      <input value={tagsInput} onChange={(e) => setTagsInput(e.target.value)} placeholder="태그 (쉼표 구분)" style={inputStyle} />
  ...
  const MeetingForm = (
    <form onSubmit={handleAddMeeting} style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
      <select value={meetingContactId} onChange={(e) => setMeetingContactId(e.target.value)}
        style={{ ...selectStyle, color: meetingContactId ? C.t0 : C.t1 }}>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        {(['positive', 'neutral', 'negative'] as MeetingMood[]).map((mood) => (
          <button key={mood} type="button" onClick={() => setMeetingMood(mood)}
            style={{
    ...
    <div style={{ fontFamily: font, display: 'flex', gap: 20, alignItems: 'flex-start' }}>
      ...
      {!isMobile && (
        <div style={{ width: 264, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {/* Form */}
          <div style={{ background: C.bg2, border: `1px solid ${C.b1}`, borderRadius: 12, padding: '14px 16px' }}>
            <p style={{ color: C.violet, fontSize: 11, fontWeight: 600, letterSpacing: '0.6px', textTransform: 'uppercase', marginBottom: 12 }}>
              {tab === 'contacts' ? '새 연락처' : '만남 기록'}
            </p>
            {tab === 'contacts' ? ContactForm : MeetingForm}
          </div>
        </div>
      )}
    </div>
