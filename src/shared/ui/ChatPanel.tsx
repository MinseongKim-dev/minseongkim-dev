...
const CHAT_INIT: Msg[] = [
  { role: 'ai', text: '안녕하세요! 오늘 일정과 할 일을 확인하고, 궁금한 것을 물어보세요.' },
];
...
export function ChatPanel() {
  ...
  const [msgs, setMsgs] = useState<Msg[]>(CHAT_INIT);
  ...
  const send = async (text?: string) => {
    ...
    setLoading(true);
    try {
      const res = await api.post<{ response: string }>('/ai/chat', { message: msg });
      setMsgs((m) => [...m, { role: 'ai', text: res.response }]);
    } catch {
        ...
        {msgs.map((m, i) => (
          <div key={i} style={{
            ...
            <div style={{
              ...
              color: C.t0, fontSize: 12.5, lineHeight: 1.6,
            }}>
              {m.text}
            </div>
          </div>
        ...
        {loading && (
          <div style={{ display: 'flex', gap: 7, alignItems: 'flex-end' }}>
            ...
            <div style={{
              ...
              borderRadius: '12px 12px 12px 4px', padding: '11px 15px', display: 'flex', gap: 5,
            }}>
              {[0, 1, 2].map((i) => (
                <div key={i} style={{ width: 5, height: 5, borderRadius: '50%', background: C.t1, opacity: 0.7 }} />
              ))}
            </div>
          </div>
        )}
